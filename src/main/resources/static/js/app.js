...
window.onload = async () => {
    ...
    // Inicializar el formulario de citas únicamente
    ...
    if (
        ...
        if (
            typeof initAppointmentAvailabilityEvents ===
            "function"
        ) {
    ...
    if (
        typeof connectNotificationWebSocket ===
        "function"
    ) {
...
async function safeRun(fn) {
    ...
    try {
        ...
        console.warn(
            `Error ejecutando ${fn.name || "función"}:`,
            error
        );
    }
}
...
function applyRoleVisibility() {
    ...
    // ADMIN + RECEPCIONISTA
    document
        .querySelectorAll(".admin-recepcion-only")
        .forEach(element => {
            element.style.display =
                role === "ADMIN" ||
                role === "RECEPCIONISTA"
    ...
    // SOLO PSICÓLOGO
    document
        .querySelectorAll(".psychologist-only")
        .forEach(element => {
            element.style.display =
                role === "PSICOLOGO"
                    ? ""
    ...
    // ADMIN + PSICÓLOGO
    document
        .querySelectorAll(".admin-psychologist-only")
        .forEach(element => {
            element.style.display =
                role === "ADMIN" ||
                role === "PSICOLOGO"
    ...
    document
        .querySelectorAll(".not-psychologist-only")
        .forEach(element => {
            element.style.display =
                role !== "PSICOLOGO"
                    ? ""
