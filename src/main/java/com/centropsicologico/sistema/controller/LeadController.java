...
if ("AGENDADO".equals(lead.getStatus())) {
    throw new RuntimeException(
            "Esta pre-reserva ya fue convertida en cita"
    );
}

if (!"PRE_RESERVADO".equals(lead.getStatus())) {
    throw new RuntimeException(
            "Solo las pre-reservas validadas pueden convertirse en cita"
    );
}

    if (lead.getServiceId() == null) {
        throw new RuntimeException(
                "La pre-reserva no tiene servicio asociado"
        );
    }

    if (lead.getPsychologistId() == null) {
        throw new RuntimeException(
                "La pre-reserva no tiene psicólogo asociado"
        );
    }
    ...
    appointment.setPaymentObservation(
            "Adelanto validado desde pre-reserva"
    );
    ...
    if (paidAmount.compareTo(BigDecimal.ZERO) > 0) {
        ...
        income.setPaymentMethod(
                lead.getPaymentMethod()
        );
